...
class FormSinhVien extends Component {
  ...
  handleOnchange = (e) => {
    ...
    // kiem tra rong
    if (value.trim() === "") {
      errorMessage = `${name} khong de trong!!`;
    }
    //  kiem tra email
    if (type === "email") {
      const regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
      if (!regex.test(value)) {
        errorMessage = ` ${name} khong dung dinh dang email`;
      }
    }
    if (name === "soDienThoai") {
      ...
      if (!regex.test(value)) {
        errorMessage = ` ${name} khong dung dinh dang /(84|0[3|5|7|8|9])+([0-9]{8})\b/`;
      }
    }
